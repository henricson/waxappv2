...
@Observable final class StoreManager {
  ...
  func fetchProducts() async {
    do {
      ...
      if sorted.isEmpty {
        let errorMsg =
          "No products found. Check: 1) Product ID matches App Store Connect 2) Paid Apps Agreement signed 3) Product is 'Ready to Submit'"
        #if DEBUG
          print("⚠️ \(errorMsg)")
        #endif
  ...
  private func bestStatus(from statuses: [Product.SubscriptionInfo.Status]) -> Product
    .SubscriptionInfo.Status?
  {
    let sorted = statuses.sorted(by: {
      (lhs: Product.SubscriptionInfo.Status, rhs: Product.SubscriptionInfo.Status) -> Bool in
