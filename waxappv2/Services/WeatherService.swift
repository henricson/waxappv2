...
private struct SnowpackState {
  ...
  var daysSinceLastMelt: Int?  // Days since temps went above 0°C (nil = no recent melt)
...
final class WeatherServiceClient {
  ...
  func fetchWeatherAndAssessSnow(for coordinate: CLLocationCoordinate2D) async throws
    -> WeatherAndSnowpackSummary
  {
  ...
  private func buildWeatherSummary(
    dailyWeather: Forecast<DayWeather>, hourlyWeather: Forecast<HourWeather>
  ) -> WeatherSummary {
    ...
    let pastDaily: [DailyHistorySummary] = recentDays.map { day in
      ...
      let avgHumidity: Double? =
        dayHours.isEmpty
        ? nil : dayHours.compactMap { $0.humidity }.reduce(0, +) / Double(dayHours.count)
  ...
  private func assessPastDailyWithHistory(pastDaily: [DailyHistorySummary])
  ...
  private func assessDay(day: DailyHistorySummary, state: inout SnowpackState)
    -> SnowSurfaceAssessment
  {
    let snowCM =
      day.snowfallAmountCM
      ?? Self.estimateSnowFromPrecip(
  ...
  private func assessCurrentConditions(
    ...
    let avgTempNext6 =
      next6Hours.map { $0.temperatureC }.reduce(0, +) / Double(max(1, next6Hours.count))

    // Check for imminent snow
    let snowImminent = next6Hours.contains { hour in
      (hour.precipitation == .snow || hour.precipitation == .mixed)
        && hour.precipitationChance >= 0.4
    }
    ...
    let wasYesterdayWet =
      yesterdayAssessment?.group == .wetCorn || yesterdayAssessment?.group == .veryWetCorn
  ...
  private static func estimateSnowFromPrecip(
    totalMM: Double?, precipitation: Precipitation?, maxTempC: Double?
  ) -> Double? {
...
extension WeatherServiceClient {
  ...
  internal func test_assessCurrentFromHourly(_ next24: [HourlyForecastEntry])
    -> SnowSurfaceAssessment?
  {
  ...
  internal func test_assessCurrentWithHistory(
    ...
    assessCurrentConditions(
      next24: next24, recentHistory: recentHistory, historicalAssessments: historicalAssessments)
  }
}
