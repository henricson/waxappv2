...
struct MagnifyingGlass<Content: View>: View {
    ...
    private var lensOverlay: some View {
        ...
        return ZStack {
            ...
            RoundedRectangle(cornerRadius: r, style: .continuous)
                .stroke(
                    LinearGradient(colors: [Color.white.opacity(0.75), Color.white.opacity(0.0)], startPoint: .topLeading, endPoint: .bottomTrailing),
                    lineWidth: 3
                )
                .blur(radius: 1.2)
                .opacity(0.55)
        ...
        GeometryReader { proxy in
            ...
            ZStack {
                content
                    ...
                    .offset(
                        x: -(location.x - sourceFrame.minX) * (scale - 1) - (location.x - sourceFrame.minX - half),
                        y: -(location.y - sourceFrame.minY) * (scale - 1) - (location.y - sourceFrame.minY - half)
                    )
                    .compositingGroup()
            ...
            .overlay {
                ...
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .stroke(
                        LinearGradient(colors: [Color.white.opacity(0.85), Color.white.opacity(0.35)], startPoint: .topLeading, endPoint: .bottomTrailing),
                        lineWidth: 1
                    )
        ...
        var body: some View {
            GeometryReader { proxy in
                ...
                ZStack {
                    ...
                    MagnifyingGlass(size: 170, scale: 5, cornerRadius: 26, location: location, coordinateSpace: "magnify-space") {
                        background
                    }
                    .position(location)
