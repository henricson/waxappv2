      ...
      VStack(spacing: 0) {
        ...
        VStack(spacing: 16) {
          VStack(spacing: 10) {
            ...
            Group {
              if !storeManager.hasAccess {
                if storeManager.isEligibleForIntroOffer {
                  Text(
                    "Start your 14-day free trial to unlock full access. Cancel anytime before the trial ends."
                  )
                } else {
          ...
          VStack(spacing: 16) {
            ...
            Text(
              "By continuing, you verify that you are at least 14 years old and agree to our Terms of Service and Privacy Policy."
            )
            .font(.caption2)
            .foregroundStyle(.secondary)
            .multilineTextAlignment(.center)
