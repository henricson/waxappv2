...
struct MapSelectView: View {
      ...
      ZStack(alignment: .bottom) {
        ...
        VStack(spacing: 0) {
          ...
          if isSearchFocused && !searchModel.suggestions.isEmpty {
            ...
            let listSuggestions =
              mostRelevant == nil
              ? searchModel.suggestions : Array(searchModel.suggestions.dropFirst())
            ...
            let measured =
              suggestionsContentHeight > 0 ? suggestionsContentHeight : suggestionsMinHeight
            ...
            TextField("Search for a place", text: $searchQuery)
              ...
              .onSubmit {
                ...
                if let top = searchModel.suggestions.first {
                  ...
                  let subtitle = top.completion.subtitle.trimmingCharacters(
  ...
  private func confirmLocation() {
    ...
    print(
      "MapSelectView: Setting manual location to (\(coordinate.latitude), \(coordinate.longitude))")
...
final class PlaceSearchModel: NSObject, ObservableObject {
  ...
  private func search(request: MKLocalSearch.Request) async -> (
    CLLocationCoordinate2D, MKCoordinateRegion
  )? {
